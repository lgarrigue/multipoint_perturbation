...
include("schrodinger_operators/schrodinger.jl")
output_path = "figs/"
...
function δα_heatmap(H0,G1,G2,range_α;log=true,fig_ttl="",expansive=false,fair=true,reso=-1)
    Gs = [G1,G2]
...
function plot_heatmaps(ex;reso=50)
    ...
    function make(fair)
        δα_heatmap(H0,G0,G1,[(-2,3),0.4];fig_ttl="very_close_not_centered",log=true,expansive=ex,fair=fair,reso=reso)
        δα_heatmap(H0,G0,G1,[(1/2,1/2),0.2];fig_ttl="very_close",log=true,expansive=ex,fair=fair,reso=reso)
        δα_heatmap(H0,G0,G1,[(1/2,1/2),0.6];fig_ttl="close",log=true,expansive=ex,fair=fair,reso=reso)
        δα_heatmap(H0,G0,G1,[(1/2,1/2),4];fig_ttl="far",log=false,expansive=ex,fair=fair,reso=reso)
        δα_heatmap(H0,G0,G1,[(1/2,1/2),50];fig_ttl="very_far",log=false,expansive=ex,fair=fair,reso=reso)
    end
    make(true)
    # make(false)
